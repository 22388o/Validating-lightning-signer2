...
#[tokio::main]
async fn main() {
    let db = if std::env::var("SCYLLA").is_ok() {
        scylla::new_and_clear().await
    } else if std::env::var("PSQL").is_ok() {
        postgres::new_and_clear().await
    } else {
        cassandra::new_and_clear().await
    }
    .unwrap();
    ...
    println!(
        ...
        ROUNDS * (CONCURRENT_TASKS - 1) * ITEMS_PER_TX * 1000 / elapsed_ms
    );

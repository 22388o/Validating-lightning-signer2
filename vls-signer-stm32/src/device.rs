...
mod device_specific {
    ...
    pub const VAL: [i32; 4] = [20, 80, 140, 80];
    pub const TXT: [i32; 6] = [40, 120, 170, 120, 80, 60];
    pub const BND: [u16; 6] = [80, 160, 10, 100, 140, 230];
}
...
mod device_specific {
    ...
    pub const VAL: [i32; 4] = [30, 160, 140, 160];
    pub const TXT: [i32; 6] = [60, 200, 170, 200, 80, 140];
    pub const BND: [u16; 6] = [80, 160, 120, 200, 10, 90];
}
...
impl Display {
    ...
    pub fn show_choice(&mut self) {
        ...
        Rectangle::new(Point::new(VAL[0], VAL[1]), Size::new(80, 80))
            .into_styled(style)
            .draw(&mut self.inner)
            .unwrap();

        Rectangle::new(Point::new(VAL[2], VAL[1]), Size::new(80, 80))
            .into_styled(style)
            .draw(&mut self.inner)
            .unwrap();
        ...
        Text::new("Yes", Point::new(TXT[0], TXT[1]), text_style).draw(&mut self.inner).unwrap();
        Text::new("No", Point::new(TXT[2], TXT[3]), text_style).draw(&mut self.inner).unwrap();
        Text::new("Choose ?", Point::new(TXT[4], TXT[5]), text_style)
            .draw(&mut self.inner)
            .unwrap();
    }
        ...
        loop {
            ...
            if num > 0 {
                ...
                match t {
                    ...
                    Ok(n) =>
                        if n.x > BND[0] && n.x < BND[1] {
                            if n.y > BND[2] && n.y < BND[3] {
                                return Ok("You pressed Yes");
                            } else if n.y > BND[4] && n.y < BND[5] {
